...
- Se uma espécie tem um tempo de geração de 5 anos, as análises devem considerar 15 anos de dados (3 gerações). Se houver um declínio de 50% nesse período, pode ser classificada pelo critério A.

- Vamos imaginar uma espécie que:
  - Começa a se reproduzir aos **3 anos**
  - Pode viver e se reproduzir até os **13 anos**
  - Produz filhotes a cada 2 anos, com igual chance em cada idade
  - A geração é a **média ponderada das idades em que os filhotes nascem**, algo como:
  > \[
  >  \text{Tempo de geração} = \frac{(3 + 5 + 7 + 9 + 11 + 13)}{6} = 8 anos
  > \]
