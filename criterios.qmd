...
- Nesse caso, o **tempo de geração** pode ser estimado de duas formas:
  ...
  **1. Método ideal (média ponderada das idades reprodutivas):**  
  ...
  > $$
  > \text{Tempo de geração} = \frac{(3 + 5 + 7 + 9 + 11 + 13)}{6} = 8 \text{ anos}
  > $$
...
O entendimento cuidadoso desse conceito é fundamental para sua correta aplicação, uma vez que um juízo construído rapidamente à respeito de sua definição pode induzir ao erro. Trata-se de um conceito operacional complexo cuja finalidade é servir como requisito para atendimento dos critérios `B` e `D`. A contagem do número de *localizações condicionadas à ameaça* representa o parâmetro a ser verificado nesses critérios.

Segundo a IUCN, o termo *localizações condicionadas à ameaça* define uma área geograficamente ou ecologicamente distinta, na qual um único evento de ameaça pode afetar **rapidamente todos os indivíduos do táxon ali presentes**.

A amplitude da *localização condicionada à ameaça* depende da área coberta pelo evento ameaçador e pode incluir parte de uma ou até várias subpopulações. Quando um táxon é afetado por mais de um evento ameaçador, a situação de ameaça deve ser definida considerando a ameaça plausível mais séria.

Os termos *localização condicionada à ameaça* e *subpopulação* são frequentemente utilizados juntos, o que leva a confusão, uma vez que ambos têm definições independentes na terminologia da IUCN [@Rivers2010]. Os termos aparecem juntos no critério `B`, por exemplo, onde declínio contínuo e flutuações extremas podem ser observados em números de subpopulações ou situação de ameaça para cumprir a condição `b` e `c`, respectivamente.

A definição de *localização condicionada à ameaça* requer um ou mais eventos ameaçadores (ameaças diretas), e o número de situações é baseado na área impactada por essas ameaças incidentes sobre partes de uma subpopulação, ou então, sobre uma ou várias subpopulações, ou partes destas.

Cada *localização condicionada à ameaça* é uma área geográfica distinta susceptível a uma ameaça mais severa. Caso haja outras subpopulações ou partes destas nas quais aquela ameaça mais severa não produza impactos, e havendo outras ameaças incidentes a estas subpopulações não afetadas, a segunda ameaça direta mais severa deverá distinguir e contabilizar as *localizações condicionadas à ameaça*. Ademais, havendo outras subpopulações não afetadas pela segunda ameaça direta mais severa e ainda houver outras ameaças incidindo sobre estas subpopulações, um terceiro evento ameaçador deverá distinguir e contabilizar as *localizações condicionadas à ameaça*, até que não haja mais eventos ameaçadores capazes de rapidamente afetar todos os indivíduos do táxon ali presentes.

Quando partes da distribuição de uma espécie **não são afetadas por nenhuma ameaça**, então é preciso definir o número de *localizações condicionadas à ameaça* nas áreas **não afetadas** com o número de subpopulações restantes. Isso implica que, apesar das diferentes definições, o número de subpopulações e *localizações condicionadas à ameaça* estão intimamente relacionados. É importante documentar o método usado para estimar o número de *localizações condicionadas à ameaça*.
...
pacotes <- c("ConR", "geobr", "sf", "leaflet")

# Verifica e instala os que não estão instalados
for (pkg in pacotes) {
...
Vamos executar um exemplo hipotético gerando pontos de coordenadas aleatórios no estado do Mato Grosso do Sul (MS) e gerar a EOO através do pacote `sf` ([*Simple Features*](https://r-spatial.github.io/sf/)) para {{< fa brands r-project >}} e visualizar o resultado com o pacote ([`leaflet`](https://rstudio.github.io/leaflet/index.html)) .

```{r}
#| warning: false

# Lista dos pacotes necessários
pacotes <- c("sf", "ggplot2", "units")
...
# 6. Visualização com o pacote leaflet
ggplot() +
    geom_sf(
      data = grid, 
      fill = "lightgray", 
...
aoo_grid <- st_transform(aoo_grid, 4326)
...
leaflet() |>
    ...
    addLayersControl(
        baseGroups = c("Mapa base", "Imagem de satélite"),
        overlayGroups = c("(Ponto único", "Buffer Quadrado (Ponto Único)"),
        options = layersControlOptions(collapsed = FALSE)
    ) |>
    addMeasure(
...
## Definicão segundo a IUCN:

**Flutuação extrema** é reconhecida quando o tamanho da população ou área de distribuição do táxon varia de forma ampla, rápida e frequente, em geral com variação maior que uma ordem de magnitude (ou seja, aumento ou diminuição de dez vezes).
:::

Há duas maneiras principais de diagnosticar flutuações extremas: 

1. Interpretando as trajetórias populacionais com base em um índice de abundância apropriado para o táxon; e 
2. Usando características de história de vida ou biologia do habitat do táxon.

#### Índice de abundância

As trajetórias populacionais devem apresentar um padrão recorrente de aumentos e diminuições (@fig-flut).

Índices de abundância em vegetação são medidas relativas que expressam a frequência, cobertura ou dominância de espécies vegetais em uma área de estudo, permitindo comparações espaciais ou temporais [@kent2012].

![Exemplos de diferentes padrões temporais de variação no tamanho populacional com o eixo Y em escala logarítmica.<br><br>(a) Flutuação irregular em torno de uma média constante, sem tendência clara de aumento ou declínio.<br>(b) Variações abruptas entre níveis discretos de abundância, mas sem direção definida (mudanças tipo "degraus").<br>(c) Ciclos regulares de aumento e queda, com retorno recorrente a níveis semelhantes de abundância.<br>(d) Flutuações com quedas pronunciadas seguidas de recuperação parcial, mas sem tendência direcional ao longo do tempo.<br>(e) Declínio contínuo e gradual na abundância populacional, sem flutuações acentuadas.<br>(f) Queda abrupta seguida de estabilização em um novo patamar mais baixo de abundância.<br>(g) Declínio gradual com flutuações regulares, refletindo uma tendência de queda combinada com variações cíclicas.<br>(h) Reduções abruptas intercaladas por períodos de relativa estabilidade ou pequenas flutuações.<br>(i) Tendência de declínio populacional com flutuações cíclicas superpostas, formando um padrão de queda escalonada.](flutuacao.png){#fig-flut}

#### Características de história de vida ou biologia do habitat

Plantas que foram mortas pelo fogo e tinham um banco de sementes esgotável armazenado no dossel (plantas <a title="a planta depende do fogo para liberar sementes, armazenando suas sementes nos frutos por longos períodos e só as liberam sob certas condições ambientais" style="text-decoration: underline;">serótinas</a> obrigatórias), por exemplo, estariam, portanto, sujeitas a flutuações extremas porque o declínio no número de indivíduos maduros representa um declínio no número total.
  - Flutuação populacional nesse cenário:
    - Antes do fogo: a população é 100% formada por adultos reprodutivos.
    - Depois do fogo: a população pode ir a 0 adultos, dependendo da existência de sementes viáveis.
    - Se sementes germinarem, leva anos até novos adultos se formarem.
    - Logo, há flutuações drásticas no número de indivíduos maduros — o que é alto risco de extinção, especialmente com incêndios frequentes.
